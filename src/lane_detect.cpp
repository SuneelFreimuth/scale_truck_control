...
Mat LaneDetector::polyfit(const vector<int>& x_val, const vector<int>& y_val) {
	Mat coef(3, 1, CV_32F);
	int N = (int)x_val.size();
	int n = 2;
	std::vector<double> x(x_val.begin(), x_val.end());
	std::vector<double> y(y_val.begin(), y_val.end());
	// double* x, * y;
	// x = new double[N];
	// y = new double[N];
	// for (int q = 0; q < N; q++) {
